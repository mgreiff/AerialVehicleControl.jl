...
function fixture()
    ...
    eps = 0.001;
    L   = 1.0;
    C  = con_state_qw_fsf_t(J, kX, kc, kw; eps=eps, L=L);

    return R, S, C, qref, wref, aref, q, w, kX, kc, kw, J, eps, L
end

@verbose(1, "Testing robust FSF on SU(2)...")
@testset "Test the continuous FSF controller on SU(2)" begin
    @testset "Good inputs" begin
        # Fixture
        Rstruct, Sstruct, Cstruct, qref, wref, aref, q, w, kX, kc, kw, J, eps, L = fixture()
        ...
        # Test
        status = ccall((:update_attitude_FSF_SU2_robust, AerialVehicleControl.CONT_LIB_PATH),
            Cint,
