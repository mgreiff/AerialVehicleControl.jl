...
Tests for the matrix_subtraction_inplace() function
"""
...
function test_call_matrix_double_subtraction_inplace(Am, Bm)
    status = ccall((:matrix_double_subtraction_inplace, AerialVehicleControl.CONT_LIB_PATH),
        Cint,
...
@verbose(1, "Testing matrix subtraction (inplace)...")
    ...
    @testset "Good inputs" begin
        for m = 1:10
            for n = 1:10
                ...
                # Call
                status = test_call_matrix_double_subtraction_inplace(Am, Bm)
                # Test
